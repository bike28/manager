...
chmod +x /usr/local/bin/menu
  ...
  echo -e "${C_YELLOW}⚠️ Applying AMOKHAN TECH SSH configuration...${C_RESET}"
...
# Download AMOKHAN TECH SSH config
...
if ! sshd -t 2>/dev/null; then
 ...
 echo -e "${C_GRAY}⚠️ SSH configuration validated.${C_RESET}"
...
if restart_ssh; then
   echo -e "${C_WHITE}⚠️ SSH service restarted.${C_RESET}"
else
...
# Run AMOKHAN TECH MANAGER setup
...
echo -e "${C_WHITE}⚠️ AUTO SCRIPT CREATED BY AMOKHAN TECH co-founder .Cyber🖥️~WHATSAPP CONTACT +255 765 556 877${C_RESET}"
