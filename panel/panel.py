...
RESELLERS_DB = "/etc/amokhantech/resellers.db"
BW_DIR = "/etc/amokhantech/bandwidth"
PANEL_CONF = "/etc/amokhantech/panel.conf"
...
def refresh_ssh_banner_config():
    ...
    if not os.path.exists("/etc/amokhantech/banners_enabled"):
        return
    sshd_ff_config = "/etc/ssh/sshd_config.d/amokhantech-banners.conf"
    banner_dir = "/etc/amokhantech/banners"
    os.makedirs(banner_dir, exist_ok=True)
    
    lines = ["# AmokhanSSH - Dynamic per-user SSH banners\n"]
    users = read_db()
    for u in users:
        un = u["username"]
...
# --- PANEL CREDS ---
def get_panel_creds():
    creds = {"PANEL_USER": "", "PANEL_PASS_HASH": "", "PANEL_PASS_PLAIN": "", "PANEL_SECRET": "", "PANEL_NAME": "AmokhanSSH", "PANEL_LOGO": "🦅"}
...
def write_panel_creds(user, pass_plain, secret=None, panel_name=None, panel_logo=None):
    ...
    if panel_name is not None:
        creds["PANEL_NAME"] = panel_name.strip() or "AmokhanSSH"
...
class PanelAPIHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        try:
            ...
            if api_path == "/api/branding":
                creds = get_panel_creds()
                self.send_json(200, {
                    "panel_name": creds.get("PANEL_NAME", "AmokhanSSH"),
                    "panel_logo": creds.get("PANEL_LOGO", "\ud83e\udd85"),
                    "has_custom_logo": os.path.exists("/etc/amokhantech/panel/logo.png") and creds.get("PANEL_LOGO") == "custom"
                })
                return

            # Serve custom logo image (public, no auth)
            logo_path = "/etc/amokhantech/panel/logo.png"
    ...
    def handle_post_trial_user(self, body, session):
        try:
            ...
            cleanup_script = "/usr/local/bin/amokhantech-trial-cleanup.sh"
    ...
    def handle_delete_user(self, username, session):
        ...
        run_cmd(f"rm -f /etc/amokhantech/banners/{username}.txt", ignore_errors=True)
        refresh_ssh_banner_config()
    ...
    def handle_put_settings(self, body):
        ...
        self.send_json(200, {"success": True, "secret": new_secret, "panel_name": new_name or creds.get("PANEL_NAME", "AmokhanSSH"), "panel_logo": new_logo or creds.get("PANEL_LOGO", "🦅")})
    ...
    def handle_delete_reseller(self, username, delete_users=False):
        ...
        if delete_users:
            ...
            for u in owned:
                ...
                run_cmd(f"rm -f /etc/amokhantech/banners/{un}.txt", ignore_errors=True)
