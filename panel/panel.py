...
# --- CONSTANTS ---
DB_FILE = "/etc/elitex/users.db"
RESELLERS_DB = "/etc/amokhan/resellers.db"
BW_DIR = "/etc/amokhan/bandwidth"
PANEL_CONF = "/etc/amokhan/panel.conf"
PANEL_HTML = "/etc/amokhan/panel/index.html"
FF_USERS_GROUP = "amokhan-users"
PORT = 44380

# --- GLOBAL STATE ---
SESSION_FILE = "/etc/amokhan/sessions.json"
...
# --- PANEL CREDS ---
def get_panel_creds():
    creds = {"PANEL_USER": "", "PANEL_PASS_HASH": "", "PANEL_PASS_PLAIN": "", "PANEL_SECRET": "", "PANEL_NAME": "Elite-X", "PANEL_LOGO": "X"}
...
def write_panel_creds(user, pass_plain, secret=None, panel_name=None, panel_logo=None):
    ...
    if panel_name is not None:
        creds["PANEL_NAME"] = panel_name.strip() or "Elite-X"
    if panel_logo is not None:
        creds["PANEL_LOGO"] = panel_logo.strip() or "X"
...
def generate_password(length=8):
    # Elite-X branded password format: elitex-### (e.g. elitex-738)
    return f"elitex-{secrets.randbelow(900) + 100}"
...
class PanelAPIHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        try:
            ...
            if api_path == "/api/branding":
                creds = get_panel_creds()
                self.send_json(200, {
                    "panel_name": creds.get("PANEL_NAME", "Elite-X"),
                    "panel_logo": creds.get("PANEL_LOGO", "X")
                })
                return
    ...
    def handle_delete_user(self, username, session):
        ...
        run_cmd(f"rm -f /etc/elitex/banners/{username}.txt", ignore_errors=True)
    ...
    def handle_put_settings(self, body):
        ...
        self.send_json(200, {"success": True, "secret": new_secret, "panel_name": new_name or creds.get("PANEL_NAME", "Elite-X"), "panel_logo": new_logo or creds.get("PANEL_LOGO", "X")})
    ...
    def handle_delete_reseller(self, username, delete_users=False):
        ...
        if delete_users:
            ...
            for u in owned:
                ...
                run_cmd(f"rm -f /etc/elitex/banners/{un}.txt", ignore_errors=True)
